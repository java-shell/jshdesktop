 ...
 * This class is strongly related to the
 * {@link jshdesktop.com.pump.swing.QDialog} project, although the
 ...
 * <P>
 * The YES_NO options should be approached with special reluctance. Microsoft
 * <A HREF="http://msdn.microsoft.com/en-us/library/aa511331.aspx">cautions</A>,
 ...
 * Delete." So instead of presenting a YES_NO dialog with the question "Do you
 * want to continue?" a better dialog might provide the options "Cancel" and
 * "Continue". In short: we as developers might tend to lazily use this option
 * and phrase dialogs in such a way that yes/no options make sense, but in fact
 * the commit buttons should be more descriptive.
 * <P>
 ...
 * one catch: on Mac the buttons are reordered: "Save", "Cancel" and "Don't
 * Save". This is to conform with standard Mac behavior. (Or, more specifically:
 * because the Apple guidelines state that a button that can cause permanent
 * data loss be as physically far from a "safe" button as possible.) On all
 * other platforms the buttons are listed in the order "Save", "Don't Save" and
 * "Cancel".
 * <P>
 * Also note the field {@link #reverseButtonOrder} controls the order each
 * option is presented in the dialog from left-to-right.
 * <h3>Platform Differences</h3> These are based mostly on studying Apple and
 * Vista interface guidelines.
 * <ul>
 ...
 * platforms the help component is a
 * {@link jshdesktop.com.pump.swing.JLink}.</li>
...
public class DialogFooter extends JPanel {
	...
	public static enum EscapeKeyBehavior {
		...
		/**
		 * This triggers the cancel button when the escape key is pressed. If no cancel
		 * button is present: this does nothing. (Also on Macs command+period acts the
		 * same as the escape key.)
		 * <p>
		 * This should only be used if the cancel button does not lead to data loss,
		 * because users may quickly press the escape key before reading the text in a
		 * dialog.
		 */
		TRIGGERS_CANCEL,
		 ...
		 * default button is defined: this does nothing. (Also on Macs command+period
		 * acts the same as the escape key.)
		 * <p>
		 * This should only be used if the default button does not lead to data loss,
		 * because users may quickly press the escape key before reading the text in a
		 * dialog.
		 */
		TRIGGERS_DEFAULT,
		 ...
		 * non-default button is defined: this does nothing. (Also on Macs
		 * command+period acts the same as the escape key.)
		 * <p>
		 * This should only be used if the non-default button does not lead to data
		 * loss, because users may quickly press the escape key before reading the text
		 * in a dialog.
		 */
		TRIGGERS_NONDEFAULT
	}
	...
	private static KeyStroke commandPeriodKey = KeyStroke.getKeyStroke(KeyEvent.VK_PERIOD,
	 ...
	 * Also this can be used as a dialog type, to indicate that "Cancel" should be
	 ...
	 * Also this can be used as a dialog type, to indicate that "OK" should be the
	 * only option presented to the user.
	 * <P>
	 * Note the usage is similar to JOptionPane's, but the numerical value is
	 ...
	 * Also this can be used as a dialog type, to indicate that "No" should be the
	 * only option presented to the user.
	 * <P>
	 * Note the usage is similar to JOptionPane's, but the numerical value is
	 ...
	 * Also this can be used as a dialog type, to indicate that "Yes" should be the
	 * only option presented to the user.
	 * <P>
	 * Note the usage is similar to JOptionPane's, but the numerical value is
	...
	/**
	 * Used to indicate a dialog should present a "Save", "Don't Save", and "Cancel"
	 * option.
	 */
	 ...
	 * Also this can be used as a dialog type, to indicate that "Save" should be the
	 * only option presented to the user.
	 */
	 ...
	 * Also this can be used as a dialog type, to indicate that "Don't Save" should
	 * be the only option presented to the user.
	 */
	public static final int DONT_SAVE_OPTION = uniqueCtr++;

	/**
	 * Used to indicate the user selected an option not otherwise specified in this
	 * set of constants. It may be possible that the user closed this dialog with
	 * the close decoration, or else another agent dismissed this dialog.
	 * <p>
	...
	private static AncestorListener escapeTriggerListener = new AncestorListener() {
		...
		private void setRootPaneContainer(JButton button, RootPaneContainer c) {
			RootPaneContainer lastContainer = (RootPaneContainer) button.getClientProperty("pump.footer.rpc");
			if (lastContainer == c)
				return;
			...
			if (c != null) {
				...
				if (JVM.isMac)
					c.getRootPane().getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put(commandPeriodKey,
							escapeKey);
			}
	 ...
	 * 
	 * @param escapeKeyIsTrigger if true then pressing the escape key will trigger
	 *                           this button. (Also on Macs command-period will act
	 *                           like the escape key.) This should be
	 *                           <code>false</code> if this button can lead to
	 *                           permanent data loss.
	 */
	...
	/**
	 * This guarantees that when the escape key is pressed (if its parent window has
	 * the keyboard focus) this button is clicked.
	 * <p>
	 * It is assumed that no two buttons will try to consume escape keys in the same
	 * window.
	 * 
	...
	/**
	 * Creates a <code>DialogFooter</code> and assigns a default button. The default
	 * button is the first button listed in the button type. For example, a
	 * YES_NO_CANCEL_OPTION dialog will make the YES_OPTION the default button.
	 * 
	 * @param options           one of the OPTIONS fields in this class, such as
	 *                          YES_NO_OPTION or CANCEL_OPTION.
	 ...
	 * 
	 * @param leftComponents    the components to put on the left side of the
	 *                          footer.
	 *                          <P>
	 *                          The Apple guidelines state that this area is
	 *                          reserved for "button[s] that affect the contents of
	 *                          the dialog itself, such as Reset [or Help]" .
	 * @param options           one of the OPTIONS fields in this class, such as
	 *                          YES_NO_OPTION or CANCEL_OPTION.
	 * @param escapeKeyBehavior one of the EscapeKeyBehavior options in this class.
	 * @return a <code>DialogFooter</code>
	 */
	public static DialogFooter createDialogFooter(JComponent[] leftComponents, int options,
	 ...
	 *                          should be the default button, or -1 if there should
	 *                          be no default button.
	 * @param escapeKeyBehavior one of the EscapeKeyBehavior options in this class.
	 * @return a <code>DialogFooter</code>
	 */
	public static DialogFooter createDialogFooter(JComponent[] leftComponents, int options, int defaultButton,
			EscapeKeyBehavior escapeKeyBehavior) {
		...
		if (options == CANCEL_OPTION || options == OK_CANCEL_OPTION || options == SAVE_DONT_SAVE_CANCEL_OPTION
		...
		if (options == DONT_SAVE_OPTION || options == SAVE_DONT_SAVE_CANCEL_OPTION
				|| options == DONT_SAVE_SAVE_OPTION) {
			dontSaveButton = createDontSaveButton((escapeKeyBehavior == EscapeKeyBehavior.TRIGGERS_NONDEFAULT
					&& defaultButton != DONT_SAVE_OPTION)
		...
		if (options == SAVE_OPTION || options == SAVE_DONT_SAVE_CANCEL_OPTION || options == DONT_SAVE_SAVE_OPTION) {
			saveButton = createSaveButton((escapeKeyBehavior == EscapeKeyBehavior.TRIGGERS_NONDEFAULT
					&& defaultButton != SAVE_OPTION)
		...
		if (options == YES_OPTION || options == YES_NO_OPTION || options == YES_NO_CANCEL_OPTION) {
			yesButton = createYesButton((escapeKeyBehavior == EscapeKeyBehavior.TRIGGERS_NONDEFAULT
					&& defaultButton != YES_OPTION)
	 ...
	 * guidelines state that "unsafe" buttons (such as "discard changes") should be
	 * several pixels away from "safe" buttons.
	 */
	 ...
	 * order. When you construct a DialogFooter, the dismiss controls should be
	 * listed in order of priority (with the most preferred listed first, the least
	 * preferred last). If this boolean is false, then those components will be
	 * listed in that order. If this is true, then those components will be listed
	 * in the reverse order.
	 * <P>
	 * By default on Mac this is true, because Macs put the default button on the
	 * right side of dialogs. On all other platforms this is false by default.
	 * <P>
	 * Window's
	 * <A HREF="http://msdn.microsoft.com/en-us/library/ms997497.aspx">guidelines
	 * </A> advise to, "Position the most important button -- typically the default
	 * command -- as the first button in the set."
	 */
		...
		private void processRootPane() {
			...
			for (int a = 0; a < dismissControls.length; a++) {
				if (dismissControls[a] instanceof JButton) {
					...
					if (ch != null) {
						KeyStroke keyStroke = KeyStroke.getKeyStroke(ch.charValue(),
								Toolkit.getDefaultToolkit().getMenuShortcutKeyMask());
		...
		private void processWindow() {
			...
			window.setFocusTraversalPolicy(new DelegateFocusTraversalPolicy(window.getFocusTraversalPolicy()) {
				...
				public Component getDefaultComponent(Container focusCycleRoot) {
					/**
					 * If the default component would naturally be in the footer *anyway*: Make sure
					 * the default component is the default button.
					 * 
					 * However if the default component lies elsewhere (a text field or check box in
					 * the dialog): that should retain the default focus.
					 * 
					 */
	 ...
	 * 
	 * @param leftControls    the controls on the left side of this dialog, such as
	 *                        a help component, or a "Reset" button.
	 * @param dismissControls the controls on the right side of this dialog that
	 *                        should dismiss this dialog. Also called "action"
	 *                        buttons.
	 * @param autoClose       whether the dismiss buttons should automatically close
	 *                        the containing window. If this is <code>false</code>,
	 *                        then it is assumed someone else is taking care of
	 *                        closing/disposing the containing dialog
	 * @param defaultButton   the optional button in <code>dismissControls</code> to
	 *                        make the default button in this dialog. (May be null.)
	 */
	public DialogFooter(JComponent[] leftControls, JComponent[] dismissControls, boolean autoClose,
	...
	private void installGUI() {
		...
		for (int a = 0; a < unsafeButtons.length; a++) {
			JComponent comp = reverseButtonOrder ? unsafeButtons[unsafeButtons.length - 1 - a] : unsafeButtons[a];
			add(comp, c);
			c.gridx++;
		...
		for (int a = 0; a < safeButtons.length; a++) {
			JComponent comp = reverseButtonOrder ? safeButtons[safeButtons.length - 1 - a] : safeButtons[a];
	 ...
	 * <P>
	 * (More specifically, this sets the <code>preferredSize</code> of each button
	 * to the largest preferred size in the list of buttons.
	 * 
	...
	public static void normalizeButtons(JButton[] buttons) {
		...
		for (int a = 0; a < buttons.length; a++) {
			...
			if ((n != null && n.intValue() == DialogFooter.DONT_SAVE_OPTION) || d.width > 80) {
				buttons[a] = null;
			}
			if (buttons[a] != null) {
	 ...
	 * 
	 * @param l this listener will be notified when a <code>dismissControl</code> is
	 *          activated.
	 */
	 ...
	 * <P>
	 * Note the components on the left side of this footer (such as a "Help" button
	 * or a "Reset Preferences" button) do NOT dismiss the dialog, and so this
	 * method has nothing to do with those components. This only relates to the
	 ...
	 * 
	 * @param buttonType of the options in this class (such as YES_OPTION or
	 *                   CANCEL_OPTION)
	 * @return the button that maps to that option, or null if no such button was
	 *         found.
	 */
	 ...
	 * <P>
	 * If this footer is recycled in different dialogs, then you may need to nullify
	 * this value for <code>getLastSelectedComponent()</code> to remain relevant.
	 */
	public void reset() {
	...
	/**
	 * Returns a copy of the <code>dismissControls</code> array used to construct
	 * this footer.
	 */
	...
	/**
	 * Returns a copy of the <code>leftControls</code> array used to construct this
	 * footer.
	 */
	...
	/**
	 * This action takes the Window associated with the source of this event, hides
	 * it, and then calls <code>dispose()</code> on it.
	 * <P>
	 * (This will not throw an exception if there is no parent window, but it does
