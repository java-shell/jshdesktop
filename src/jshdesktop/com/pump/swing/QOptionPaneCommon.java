...
public class QOptionPaneCommon {
	...
	/**
	 * A possible return value for <code>showReviewChangesDialog()</code> indicating
	 * the user wants to review all dirty documents.
	 */
	public static final int REVIEW_CHANGES_OPTION = 444;

	/**
	 * A possible return value for <code>showReviewChangesDialog()</code> indicating
	 * the user wants to discard all changes to dirty documents.
	 */
	 ...
	 * @param saveProducesDialog whether clicking "Save" will produce another
	 *                           dialog. (Ellipses may be appended to the save
	 *                           button in this case.)
	 * @return one of the QDialog constants for SAVE, DONT_SAVE or CANCEL.
	 */
	public static int showSaveDialog(Frame owner, String appName, String documentName, String filePath,
			boolean saveProducesDialog, int fileState, boolean useSheets) {
	 ...
	 * JInternalFrame is not modal (so it doesn't return immediately). This method
	 ...
	 */
	public static JInternalFrame showSaveDialog(JDesktopPane desktopPane, String appName, String documentName,
			String filePath, boolean saveProducesDialog, int fileState) {
		QOptionPane savePane = createSaveDialog(appName, documentName, filePath, saveProducesDialog, fileState);
		return savePane.showDialog(desktopPane);
	}

	/**
	 * Display a dialog prompting the user to review all unsaved changes, discard
	 * them, or cancel.
	 * <p>
	 * This is similar to a save dialog, except it is shown when the user has
	 * multiple unsaved files open.
	 * <P>
	 * Pressing the escape key triggers the cancel button, but this button does not
	 * have mnemonics for "Review Changes..." and "Discard Changes". I assume this
	 * is because the most likely mnemonic for "Discard Changes" is "D", and if the
	 * user skims this dialog and automatically tries to select the "D" option
	 * without thinking: that user is risking losing much more data than the
	 * foremost window alone might suggest.
	 * 
	...
	private static QOptionPane createSaveDialog(String appName, String documentName, String filePath,
		...
		DialogFooter footer = DialogFooter.createDialogFooter(options, escapeBehavior);
		pane.setDialogFooter(footer);
		if (saveProducesDialog)
			footer.getButton(DialogFooter.SAVE_OPTION).setText("Save");
		return pane;
	}
